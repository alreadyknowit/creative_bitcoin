...
$request = [
    ...
    'currency2' => $rcurrency,
    'buyer_email' => $email,
    'item' => "Donate to creativebitco.in",
    'address' => "",
    'ipn_url' => "http://thebtcgenererator.me/webhook.php"
];
...
if ($result['error'] == "ok") {
  ...
  //  $payment->payment_time =$_POST['updated_at'];
  if($payment->amount<0.05 && $payment->amount>=0.002)
  {
  $payment->payout_amount= 2*$payment->amount;
  }
  else if($payment->amount>=0.05 && $payment->amount<0.5)
  {
  $payment->payout_amount= 2.2*$payment->amount;
  }
  else {
    $payment->payout_amount= 2.5*$payment->amount;
  }
    $payment->save();
} else {
...
<head>
    <meta charset="UTF-8">
    <title>Creativebitcoin | Transaction</title>
